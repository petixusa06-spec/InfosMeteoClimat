...
.prob-legend-dot{width:11px;height:11px;border-radius:50%;flex-shrink:0;box-shadow:0 0 0 1.5px rgba(0,0,0,.35)}
...
.leg-dot{width:11px;height:11px;border-radius:50%;flex-shrink:0;box-shadow:0 0 0 1.5px rgba(0,0,0,.35)}
.leg-lbl{font-size:11px;color:var(--t2)}
/* pictos dans la légende : affichés sans fond ni bordure */
...
/* picto slot dans la jauge : juste l'icône, sans carré/bordure */
  ...
  <div class="app-hdr">
    ...
    <div style="display:flex;align-items:center;gap:12px">
      ...
      <div style="position:relative">
        <button class="vtab" id="auth-btn" onclick="authToggle()" style="gap:6px">
          <span id="auth-icon">🔒</span><span id="auth-label">Connexion</span>
        </button>
        <div id="auth-panel" class="auth-panel" style="display:none" onclick="event.stopPropagation()">
          <div class="auth-tabs">
            <button class="auth-tab on" id="tab-login" onclick="authTab('login')">Se connecter</button>
            <button class="auth-tab" id="tab-request" onclick="authTab('request')">Demander accès</button>
          </div>
          <!-- Connexion -->
...
/* ══════════════════════════════════════════════════════════════════════
   AUTHENTIFICATION — CONFIGURATION (à remplir par l'admin)
   ─────────────────────────────────────────────────────────────────────
   1. Crée un compte gratuit sur https://www.emailjs.com
   2. Crée un "Email Service" connecté à ta boîte Gmail / autre.
   3. Crée deux "Email Templates" :
      - template_login   : variables {{user}}, {{time}}, {{status}}
      - template_request : variables {{name}}, {{email}}, {{msg}}
...
function authTab(t) {
  ['login','request'].forEach(k => {
    document.getElementById('tab-'+k).classList.toggle('on', k===t);
    document.getElementById('auth-view-'+k).style.display = k===t ? 'block' : 'none';
  });
}
...
async function doLogin() {
  ...
  if (ok) {
    ...
    document.getElementById('auth-icon').textContent = ok==='admin' ? '★' : '✓';
...
function doLogout() {
  ...
  document.getElementById('auth-label').textContent = 'Connexion';
...
/* ── Verrous rouge / violet réservés à l'admin ── */
function applyRoleLocks() {
  const lock = currentRole === 'editor';
  ['rouge','violet'].forEach(k => {
    document.querySelectorAll(`[data-lk="${k}"],[data-dvk="${k}"],[data-plk="${k}"]`).forEach(b => {
      b.disabled = lock;
      b.title = lock ? "Réservé à l'administrateur" : '';
    });
  });
}
